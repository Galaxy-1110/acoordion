...
export default function Home() {
	...
	return (
		<main className="min-h-dvh justify-center flex p-2 align-middle items-center ">
			<div className="bg-white max-h-fit lg:w-3/4 md:p-14 md:pl-0 p-10 rounded-2xl flex md:flex-row flex-col relative object-contain drop-shadow-2xl  transition-all ease-linear duration-500 overflow-hidden gap-9">
						...
						{question.map((data, i) => {
							return (
								<div className="flex flex-col gap-4" key={i}>
									<div
										className={
											"flex justify-between items-center cursor-pointer hover:text-orange-300 transition-all ease-in-out"
										}
										onClick={() => {
											handleClick(i);
											focus();
										...
										<h3
											className={
												"text-lg text-inherit transition-all ease-in-out" +
										...
										<img
											...
											className={
												"w-[14px] h-[10px]  cursor-pointer transition-all ease-in-out" +
												(currentOpen === i
									...
									<div
										className={
											"text-gray-500  overflow-hidden grid transition-all ease-in-out duration-[500ms] " +
											(currentOpen === i ? "grid-rows-[1fr]" : "grid-rows-[0]")
										}>
										{data.answer}
									</div>
